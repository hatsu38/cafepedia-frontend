  ...
  <div>
    <v-card outlined max-width="700" class="mx-auto">
      ...
      <v-card-text class=" pt-0 mt-n5">
        <v-row justify="space-around" align="center" class="pt-0">
          <v-col cols="auto" class="display-2 pa-0">
            ...
            <span :class="`${cafe.main_shop_eng_name}-title`">{{
              cafe.distance
            }}</span>
          </v-col>
      ...
      <p class="mb-0 mx-2 pb-2 text-right body-2">
        ...
        >
          <v-icon color="blue darken-2" style="font-size: 14px;">
            fas fa-external-link-alt
          </v-icon>
          情報の修正を提案
        </a>
      </p>
      <v-container v-if="snackbar == false">
        <h4 class="subtitle-1 mb-1">
          今の混雑情報を教えてください
        </h4>
        <v-row justify="space-around" align="center">
          <v-col cols="auto" justify="center" class="pa-1">
            <v-btn small dark color="blue lighten-3" @click="submit(1)">
              <v-icon small>
          ...
          <v-col cols="auto" justify="center" class="pa-1">
            <v-btn small dark color="amber darken-1" @click="submit(2)">
              <v-icon small>
          ...
          <v-col cols="auto" justify="center" class="pa-1">
            <v-btn small dark color="red" @click="submit(3)">
              <v-icon small>
                fa-fw fas fa-users
              </v-icon>
              激混み
...
export default {
  ...
  async created() {
    ...
    const res = await axios.get(
      `${this.$urls.apiUrl}api/shops/${this.$nuxt.$route.params.id}`,
      { params: query }
    )
  ...
  methods: {
    createMetaDescription(cafe) {
      ...
      const stationText = this.$nuxt.$route.query.stationName
        ? this.$nuxt.$route.query.stationName + '近くにある'
        : ''
    ...
    editUrlRequest(cafe) {
      const cafeName = 'entry.537099549=' + cafe.name,
        cafeAddress = cafe.other_address
          ? '&entry.1935919079=' +
            cafe.prefecture +
            cafe.city +
            cafe.other_address
        ...
        cafeBusinessHour = cafe.business_hour
          ? '&entry.1300411853=' + cafe.business_hour
          : '',
      ...
      const googleForm =
        'https://docs.google.com/forms/d/e/1FAIpQLSdkTXNrlKD-6fFP7EvBwU0DqqiWR_7J_4LAO6Igs-_7KyxiMw/viewform?usp=sf_link&'
      const googleURL =
        googleForm +
        cafeName +
        cafeAddress +
    ...
    async submit(congrestion_info) {
      ...
      const res = await axiosPost.post(
        `${this.$urls.apiUrl}api/shops/${this.$nuxt.$route.params.id}/congrestion_infos`,
        {
          congrestion_infos: {
            id: congrestion_info
          }
    ...
    async del_submit() {
      if (this.shopCongrestionInfoId) {
        await axiosPost.delete(
          `${this.$urls.apiUrl}api/shop_congrestion_infos/${this.shopCongrestionInfoId}`
        )
      }
      this.snackbar = false
  ...
  head() {
    return {
      title:
        this.cafe.name +
        ' | カフェぺディア | あなたの近くにあるカフェがすぐに見つかる',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.createMetaDescription(this.cafe)
        }
      ]
    }
  }
